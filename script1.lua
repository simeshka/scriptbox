...
filler.LayoutOrder = 5
filler.BackgroundTransparency = 1
filler.BorderSizePixel = 0
filler.Parent = frame
local noclipButton = Instance.new("TextButton")
noclipButton.LayoutOrder = 6
...
local function updateSpeedLabel(messageOverride)
	if messageOverride then
		speedLabel.Text = messageOverride
	else
		local currentSpeed = computeSpeedFromLevel(speedLevel)
		speedLabel.Text = string.format("Speed: %.1f", speedLevel)
	end
end
updateSpeedLabel()
...
minusButton.MouseButton1Click:Connect(function()
	if speedLevel > minLevel then
		...
		local currentSpeed = computeSpeedFromLevel(speedLevel)
		updateSpeedLabel(string.format("Speed cannot be below 1"))
...
RunService.RenderStepped:Connect(function()
	...
	if horizontal.Magnitude > 0 and camFlat.Magnitude > 0 then
		forwardAmount = horizontal:Dot(camFlat)
	end
	local vertical = look.Y * forwardAmount
	local finalDir = Vector3.new(horizontal.X, vertical, horizontal.Z)
	if finalDir.Magnitude < 0.05 then
		hrp.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
		return
	end
	finalDir = finalDir.Unit
	local currentSpeed = computeSpeedFromLevel(speedLevel)
	hrp.AssemblyLinearVelocity = finalDir * currentSpeed
end)
